...
class BlogController {
    ...
    async deleteImg(ctx) {
        try {
            ...
            if (
                blog.featureImage.avatarID &&
                (await removeDirectory(`upload/${blog.featureImage.avatarID}`))
            ) {
    ...
    async getAllUserBlogs(ctx) {
        ...
        try {
            const blogs = await Blog.find({ postedBy: userId })
                ...
                .limit(perPage)
                .select(
                    '_id title excerpt slug featureImage visited tags postedBy published createdAt'
                )
            return (ctx.body = {
                blogs: blogs,
    ...
    async getBlog(ctx) {
        try {
            // we are using fineOneAndUpdate to add visited +1 to DB
            return (ctx.body = await Blog.findOne({ slug: ctx.params.slug })
                .populate('categories', '_id name slug')
                .populate('tags', '_id name slug')
                .populate('postedBy', '_id name username')
                .select(
