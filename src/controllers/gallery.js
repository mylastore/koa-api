...
const DOMAIN = process.env.NODE_ENV === 'development' ? process.env.DEV_PATH : process.env.PRODUCTION_PATH
...
class GalleryController {
  ...
  async saveGallery(ctx, next) {
    ...
    try {
      ...
      return ctx.body = await Gallery.findOneAndUpdate({_id: galleryID }, data, {new: true})
    } catch (err) {
      ctx.throw(422, err)
    }
  }
  ...
  async updateGallery(ctx) {
    ...
    try {
      return ctx.body = await Gallery.findOneAndUpdate({_id: galleryID }, {$push: {images: images}}, {new: true})
    } catch (err) {
      ctx.throw(422, err)
    }
  }
  ...
  async deleteImage(ctx){
    ...
    try {
      removeFile(path)
      ctx.body =  await Gallery.findOneAndUpdate({_id: galleryID }, {$pull: {images: { _id: id } } }, {safe: true, upsert: true, new: true} )
    }catch (err){
      ctx.throw(422, err)
    }
  ...
  async publishedGallery(ctx){
    ...
    try {
      await Gallery.findOneAndUpdate({_id: body._id }, {published: body.published}, {new: true})
  ...
  async getPublishedGalleries(ctx){
    try{
      ctx.body = await Gallery.find({published: true})
        .sort({'createdAt': -1})
    }catch (err){
      ctx.throw(422, err)
    }
  ...
  async deleteGallery(ctx){
    ...
    try{
      ...
      if(res){
        ...
        return ctx.body = {status: 200, message: "Gallery was deleted"}
      }
    }catch (err){
      ctx.throw(422, err)
    }
